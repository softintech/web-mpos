...
export function SaleProductsFilter({ listLoading,name}) {
  ...
  const applyFilter = (values) => {
    const newQueryParams = prepareFilter(SaleProductsUIProps.queryParams, values);
    if (!isEqual(newQueryParams, SaleProductsUIProps.queryParams)) {
      newQueryParams.pageNumber = 1;
      // update list by queryParams
      SaleProductsUIProps.setQueryParams(newQueryParams);
    }
  };
