...
export function ResultsFilter({ listLoading,name}) {
  // Results UI Context
  ...
  const applyFilter = (values) => {
    const newQueryParams = prepareFilter(ResultsUIProps.queryParams, values);
    if (!isEqual(newQueryParams, ResultsUIProps.queryParams)) {
      newQueryParams.pageNumber = 1;
      // update list by queryParams
      ResultsUIProps.setQueryParams(newQueryParams);
    }
  };
              ...
              <div className="col-lg-6" >
                <div className="float-sm-right">
                  <button
                      ...
                      onClick={ResultsUIProps.newShopsPageCreshopButtonClick}
                      >{name}
                  </button>
                </div>
