...
export function StockCountsFilter({ listLoading,name}) {
  ...
  const applyFilter = (values) => {
    const newQueryParams = prepareFilter(StockCountsUIProps.queryParams, values);
    if (!isEqual(newQueryParams, StockCountsUIProps.queryParams)) {
      newQueryParams.pageNumber = 1;
      // update list by queryParams
      StockCountsUIProps.setQueryParams(newQueryParams);
    }
  };
              ...
              <div className="col-lg-6" >
                <div className="float-sm-right">
                  <button
                      ...
                      onClick={StockCountsUIProps.newShopsPageCreshopButtonClick}
                      >{name}
                  </button>
                </div>
