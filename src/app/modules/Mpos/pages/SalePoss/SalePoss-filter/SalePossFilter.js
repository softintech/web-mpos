...
export function SalePossFilter({ listLoading,name}) {
  // SalePoss UI Context
  ...
  const applyFilter = (values) => {
    const newQueryParams = prepareFilter(SalePossUIProps.queryParams, values);
    if (!isEqual(newQueryParams, SalePossUIProps.queryParams)) {
      newQueryParams.pageNumber = 1;
      // update list by queryParams
      SalePossUIProps.setQueryParams(newQueryParams);
    }
  };
              ...
              <div className="col-lg-6" >
                <div className="float-sm-right">
                  <button
                      ...
                      onClick={SalePossUIProps.newShopsPageCreshopButtonClick}
                      >{name}
                  </button>
                </div>
