...
export function PuyPursFilter({ listLoading,name}) {
  // PuyPurs UI Context
  ...
  const applyFilter = (values) => {
    const newQueryParams = prepareFilter(PuyPursUIProps.queryParams, values);
    if (!isEqual(newQueryParams, PuyPursUIProps.queryParams)) {
      newQueryParams.pageNumber = 1;
      // update list by queryParams
      PuyPursUIProps.setQueryParams(newQueryParams);
    }
  };
              ...
              <div className="col-lg-6" >
                <div className="float-sm-right">
                  <button
                      ...
                      onClick={PuyPursUIProps.newShopsPageCreshopButtonClick}
                      >{name}
                  </button>
                </div>
