...
export function PuyMatsFilter({ listLoading,name}) {
  // PuyMats UI Context
  ...
  const applyFilter = (values) => {
    const newQueryParams = prepareFilter(PuyMatsUIProps.queryParams, values);
    if (!isEqual(newQueryParams, PuyMatsUIProps.queryParams)) {
      newQueryParams.pageNumber = 1;
      // update list by queryParams
      PuyMatsUIProps.setQueryParams(newQueryParams);
    }
  };
              ...
              <div className="col-lg-6" >
                <div className="float-sm-right">
                  <button
                      ...
                      onClick={PuyMatsUIProps.newShopsPageCreshopButtonClick}
                      >{name}
                  </button>
                </div>
