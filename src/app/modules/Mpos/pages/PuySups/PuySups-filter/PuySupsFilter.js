...
export function PuySupsFilter({ listLoading,name}) {
  // PuySups UI Context
  ...
  const applyFilter = (values) => {
    const newQueryParams = prepareFilter(PuySupsUIProps.queryParams, values);
    if (!isEqual(newQueryParams, PuySupsUIProps.queryParams)) {
      newQueryParams.pageNumber = 1;
      // update list by queryParams
      PuySupsUIProps.setQueryParams(newQueryParams);
    }
  };
              ...
              <div className="col-lg-6" >
                <div className="float-sm-right">
                  <button
                      ...
                      onClick={PuySupsUIProps.newShopsPageCreshopButtonClick}
                      >{name}
                  </button>
                </div>
