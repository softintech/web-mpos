...
export function ShopsCreEmpsFilter({ listLoading,name}) {
  ...
  const applyFilter = (values) => {
    const newQueryParams = prepareFilter(ShopsCreEmpsUIProps.queryParams, values);
    if (!isEqual(newQueryParams, ShopsCreEmpsUIProps.queryParams)) {
      newQueryParams.pageNumber = 1;
      // update list by queryParams
      ShopsCreEmpsUIProps.setQueryParams(newQueryParams);
    }
  };
