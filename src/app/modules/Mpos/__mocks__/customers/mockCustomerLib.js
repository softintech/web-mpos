...
export const CUSTOMERS_URL = "api/esss";

export async function AjaxDataCustomer(resove = "") {
  const idapp = localStorage.getItem("idapp");

  let _essGroup = parseInt(idapp);
  //const data = await new Promise((r, j) => ajax.Get(`${hosts}/customer/all/${_essGroup}`, Token, '', r));

  new Promise((r, j) => {
    ajax.Get(`${hosts}/customer/all/${_essGroup}`, Token, "", r);
  }).then(data => {
    if (data != false) {
...
export const update = (id, data, userId, r = "") => {
  ...
  const res = new Promise((r, j) =>
    ajax.Post(`${hosts}/customer/update`, Token, obj, r)
  );
  res.then(v => {
    console.log(v);
    if (v != false) {
...
export const deletedb = (id, r = "") => {
  ...
  const res = new Promise((r, j) =>
    ajax.Post(`${hosts}/customer/delete`, Token, obj, r)
  );
  res.then(v => {
    console.log(v);
    if (v != false) {
